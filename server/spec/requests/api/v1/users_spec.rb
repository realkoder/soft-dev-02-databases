...
RSpec.describe Api::V1::UsersController, type: :request do
  ...
  let!(:auth_user) { Relational::User.create!(first_name: 'Jane', last_name: 'Smith', email: 'jane@example.com', password: 'secure123') }
  let(:headers) do
  ...
  context '#create' do
    ...
    it 'creates a new user with valid attributes and without sensitive data' do
      expect { post '/api/v1/users', params: { user: valid_attributes }.to_json, headers: { 'Content-Type' => 'application/json' } }.to change(Relational::User, :count).by(1)
    ...
    it 'creates a new user only one time if tried multiple' do
      expect { post '/api/v1/users', params: { user: valid_attributes }.to_json, headers: { 'Content-Type' => 'application/json' } }.to change(Relational::User, :count).by(1)
      expect { post '/api/v1/users', params: { user: valid_attributes }.to_json, headers: { 'Content-Type' => 'application/json' } }.to change(Relational::User, :count).by(0)
