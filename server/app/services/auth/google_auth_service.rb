class Auth::GoogleAuthService
  ...
  def self.validate_code_and_get_user(code, use_db)
    client_id = ENV["GOOGLE_CLIENT_ID"]
    ...
    user = if use_db == 'mongodb'
             Document::User.find_or_initialize_by(email: user_info['email'])
           elsif use_db == 'neo4j'
             Graph::User.find_or_initialize_by(email: user_info['email'])
           else
